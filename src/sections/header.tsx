...
import { activeSectionSignal, timeOfLastClickSignal } from "../signals/active-section-signal";
                ...
                className={clsx(
                  "flex w-full items-center justify-center px-3 py-3 hover:text-gray-950 transition dark:text-gray-300 dark:hover:text-gray-300",
                  {
                    "text-gray-950 dark:text-white":
                      activeSectionSignal.value === link.name,
                  }
                )}
                href={link.hash}
                onClick={() => {
                  activeSectionSignal.value = link.name;
                timeOfLastClickSignal.value = Date.now();
                }}
              >
                {link.name}

                {link.name === activeSectionSignal.value && (
                  <motion.span
